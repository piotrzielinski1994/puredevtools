...
- Each rule matches by URL (glob or regex) and HTTP method. The editor has three tabs: **Match** (URL/method), **Request** (`set`/`remove` request headers and/or rewrite the request body, applied before the request is forwarded), and **Response** (`set`/`remove` response headers and/or rewrite the response body with an optional content-type, applied to the forwarded response). The real request is always forwarded and the original status preserved.
