...
export const createPatchedXhr = (deps: PatchedXhrDeps): typeof XMLHttpRequest => {
  class PatchedXhr {
    ...
    private async sendWithPreScript(
      ...
      (this.delegate.open as (m: string, u: string) => void)(mutable.method, mutable.url);
      mutable.headers.forEach((value, name) => this.delegate.setRequestHeader(name, value));
      const forwardBody = mutable.body ?? (typeof body === 'string' ? undefined : body);
      this.requestBody = typeof forwardBody === 'string' ? forwardBody : this.requestBody;
    ...
    private wire(interception: Interception): void {
      ...
      delegate.onload = (event) => {
        const fire = (): void => void this.onload?.call(this as unknown as XMLHttpRequest, event);
    ...
    private async applyOverride(interception: Extract<Interception, { kind: 'override' }>): Promise<void> {
      const delegate = this.delegate;
      ...
      if (interception.postScript !== undefined) {
        ...
        const mutable: MutableResponse = { status: delegate.status, headers: scriptHeaders, body };
