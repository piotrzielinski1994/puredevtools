...
const mutableResponse = (
  overrides: Partial<MutableResponse> = {},
): MutableResponse => ({
  status: 200,
  headers: new Headers(),
  body: "",
...
describe("createRequestFacade (AC-004)", () => {
  it("should read the underlying url and method through the getters", () => {
    ...
    const facade = createRequestFacade(
      mutableRequest({ url: "https://api.x/y", method: "POST" }),
    );
  ...
  it("should read a header case-insensitively (AC-004)", () => {
    ...
    const facade = createRequestFacade(
      mutableRequest({ headers: new Headers({ "X-Env": "staging" }) }),
    );
  ...
  it("should return all headers as a plain record", () => {
    ...
    const facade = createRequestFacade(
      mutableRequest({ headers: new Headers({ "X-A": "1", "X-B": "2" }) }),
    );
  ...
  it("should return the raw string from getBody when the body is not valid JSON", () => {
    ...
    const facade = createRequestFacade(
      mutableRequest({ body: "name=piotr&x=1" }),
    );
...
describe("createResponseFacade (AC-005)", () => {
  ...
  it("should read a response header case-insensitively", () => {
    const facade = createResponseFacade(
      mutableResponse({
  ...
  it("should set and remove response headers, readable back and on the underlying response", () => {
    ...
    const res = mutableResponse({
      headers: new Headers({ "Set-Cookie": "sid=1" }),
    });
  ...
  it("should return all response headers as a plain record", () => {
    const facade = createResponseFacade(
      mutableResponse({ headers: new Headers({ "X-A": "1" }) }),
    );
  ...
  it("should parse a JSON body into an object via getBody", () => {
    // behavior: getBody returns parsed JSON when the body is valid JSON
    const facade = createResponseFacade(
      mutableResponse({ body: '{"id":42,"ok":true}' }),
    );
  ...
  it("should return undefined (not throw) from getJson on a non-JSON body (AC-005)", () => {
    ...
    const facade = createResponseFacade(
      mutableResponse({ body: "not json <html>" }),
    );
...
describe("createConsoleFacade (AC-011)", () => {
  it("should forward log args to the sink with the puredevtools script prefix", () => {
    ...
    expect(
      args.some(
        (a) => typeof a === "string" && a.includes("[puredevtools script]"),
      ),
    ).toBe(true);
