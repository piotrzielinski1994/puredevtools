...
describe('resolveRewrite (AC-002)', () => {
  it('should swap only scheme/host/port and keep the original path and query for an origin-only target (TC-001)', () => {
    ...
    expect(resolveRewrite('https://api.prod.com/users/1?x=2', 'http://localhost:3000')).toBe(
      'http://localhost:3000/users/1?x=2',
    );
  });
  ...
  it('should keep the original hash as well as the path and query on an origin swap (TC-001)', () => {
    ...
    expect(resolveRewrite('https://api.prod.com/users/1?x=2#frag', 'http://localhost:3000')).toBe(
      'http://localhost:3000/users/1?x=2#frag',
    );
  });
  ...
  it('should full-replace the path while preserving the original query when the target omits one (TC-002)', () => {
    ...
    expect(resolveRewrite('https://api.prod.com/users/1?x=2', 'http://localhost:3000/mock')).toBe(
      'http://localhost:3000/mock?x=2',
    );
  });
  ...
  it('should preserve the original hash on a full replace when the target omits it (TC-002)', () => {
    ...
    expect(resolveRewrite('https://api.prod.com/users/1?x=2#frag', 'http://localhost:3000/mock')).toBe(
      'http://localhost:3000/mock?x=2#frag',
    );
  });
  ...
  it('should let the target query win and drop the original query when the target carries its own (TC-004)', () => {
    ...
    expect(resolveRewrite('https://api.prod.com/users/1?x=2', 'http://localhost:3000/mock?y=9')).toBe(
  ...
  it('should return the original url unchanged for an empty target (TC-005)', () => {
    ...
    expect(resolveRewrite('https://api.prod.com/users/1?x=2', '')).toBe('https://api.prod.com/users/1?x=2');
  });
  ...
  it('should return the original url unchanged for an unparseable target without throwing (TC-005)', () => {
    ...
    expect(() => resolveRewrite('https://api.prod.com/users/1?x=2', 'http://localhost:99999')).not.toThrow();
    expect(resolveRewrite('https://api.prod.com/users/1?x=2', 'http://localhost:99999')).toBe(
  ...
  it('should full-replace on the same origin for a root-relative target (TC-006)', () => {
    ...
    expect(resolveRewrite('https://api.prod.com/users/1?x=2', '/mock')).toBe('https://api.prod.com/mock?x=2');
  });
  ...
  it('should resolve a protocol-relative target against the original scheme, keeping the path/query (edge, AC-002)', () => {
    ...
    expect(resolveRewrite('https://api.prod.com/users/1?x=2', '//cdn.example.com')).toBe(
      'https://cdn.example.com/users/1?x=2',
    );
  });
});
