...
import { safeNormalize, resolveShortcuts, findConflict } from './resolve';
...
describe('resolveShortcuts', () => {
  ...
  it('should fall back to the default if an override value is not an array', () => {
    ...
    const def = SHORTCUT_ACTIONS.find((a) => a.id === 'save-rule')!.defaultHotkey;
  ...
  it('should not throw on a corrupt overrides map', () => {
    const overrides = { 'save-rule': 42, bogus: 'Mod+Q' } as unknown as ShortcutOverrides;
