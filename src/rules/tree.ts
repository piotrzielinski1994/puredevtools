...
export const treeOps = createTreeOps<RuleNode, Rule>({
  ...
  cloneLeaf: (leaf, newId, renameTop) => ({
    kind: 'rule',
    rule: copyRule(leaf.rule, newId, renameTop ? `${leaf.rule.name} (copy)` : leaf.rule.name),
  }),
});
...
export const findNode = (tree: TreeNode[], id: string): TreeNode | undefined => ops.findNode(tree, id);
export const containsId = (node: TreeNode, id: string): boolean => ops.containsId(node, id);
...
export const toggleCollapse = (tree: TreeNode[], id: string): TreeNode[] => ops.toggleCollapse(tree, id);
export const walkRuleIds = (tree: TreeNode[]): string[] => ops.walkLeafIds(tree);
export const updateRuleInTree = (tree: TreeNode[], rule: Rule): TreeNode[] => ops.updateLeafInTree(tree, rule);
export const collectFolderIds = (tree: TreeNode[]): Set<string> => ops.collectFolderIds(tree);
...
export const duplicateNode = (tree: TreeNode[], id: string): TreeNode[] => ops.duplicateNode(tree, id);
