import type { Matchers, PatternKind, RequestDescriptor, Rule } from "./model";
...
export const matchesRequest = (
  ...
  const url = matchUrl(
    rule.matchers.url.pattern,
    rule.matchers.url.kind,
    request.url,
  );
  ...
  return {
    ok: true,
    matched: matchesMethod(rule.matchers.methods, request.method),
  };
};
