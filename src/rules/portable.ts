...
import { type PortableState, portableSchema } from "./schema";
  ...
  return {
    ...
    rule: {
      ...
      actions: node.rule.actions.map((action) =>
        action.type === "rewriteBody"
          ? { ...action, body: bodyToDisk(action.body) }
          : action,
      ),
    },
  };
...
const decodeNode = (node: unknown): unknown => {
  ...
  if (
    node.kind === "rule" &&
    isRecord(node.rule) &&
    Array.isArray(node.rule.actions)
  ) {
    return {
      ...node,
      rule: { ...node.rule, actions: node.rule.actions.map(decodeAction) },
    };
  }
  return node;
};
...
export const exportRules = (state: PortableState): string =>
  JSON.stringify(
    { enabled: state.enabled, workspace: state.workspace.map(encodeNode) },
    null,
    2,
  );
