...
import { globToRegExp, matchesRequest, matchUrl } from "./match";
import type { Matchers, RequestDescriptor, Rule, RuleAction } from "./model";
...
const expectMatched = (
  result: ReturnType<typeof matchUrl>,
  expected: boolean,
) => {
...
describe("matchUrl", () => {
  ...
  it("should not throw when given an invalid regex pattern", () => {
    expect(() =>
      matchUrl("([", "regex", "https://api.example.com/users"),
    ).not.toThrow();
  });
...
describe("matchesRequest", () => {
  ...
  it("should match any method if no methods are specified (TC-007)", () => {
    ...
    expectMatched(
      matchesRequest(rule, buildRequest({ method: "DELETE" })),
      true,
    );
  });
  ...
  it("should treat an empty methods array as match-any", () => {
    ...
    expectMatched(
      matchesRequest(rule, buildRequest({ method: "PATCH" })),
      true,
    );
  });
  ...
  it("should match any request if only url is specified and it matches", () => {
    ...
    expectMatched(
      matchesRequest(rule, buildRequest({ method: "OPTIONS" })),
      true,
    );
  });
