...
export type TreeKeyboardDeps<Leaf extends { kind: string }> = {
  ...
  locateNode(tree: Array<Leaf | TreeFolder<Leaf>>, id: string): NodeLocation | null;
};

export const createTreeKeyboard = <Leaf extends { kind: string }>(deps: TreeKeyboardDeps<Leaf>) => {
  type Node = Leaf | TreeFolder<Leaf>;
  const isFolder = (node: Node): node is TreeFolder<Leaf> => node.kind === 'folder';
  ...
  const flattenSelectable = (tree: Node[], expandedIds: Set<string>): string[] =>
    tree.flatMap((node) => {
  ...
  const treeMoveTarget = (tree: Node[], id: string, direction: TreeMoveDirection): MoveTarget | null => {
  ...
  const commandFor = (
    ...
    if (action === 'tree-move-up' || action === 'tree-move-down' || action === 'tree-outdent' || action === 'tree-nest') {
      const direction: TreeMoveDirection =
        action === 'tree-move-up' ? 'up' : action === 'tree-move-down' ? 'down' : action === 'tree-outdent' ? 'outdent' : 'nest';
    ...
    if (action === 'tree-activate') {
      return isFolder(node) ? { type: 'toggle', id: focusedId } : { type: 'activate', id: focusedId };
    }
    ...
    if (action === 'tree-collapse') {
      if (isFolder(node) && expandedIds.has(focusedId)) return { type: 'collapse', id: focusedId };
  ...
  const resolveTreeKey = (input: {
    ...
    const action = TREE_ACTION_ORDER.find((id) => {
      ...
      return Array.isArray(actionBindings) && actionBindings.some((binding) => matchesKeyboardEvent(event, binding as Hotkey));
    });
    if (!action) return NONE;
    return commandFor(action, tree, expandedIds, focusedId, node);
  };

  return { expandedFolderIds, flattenSelectable, treeMoveTarget, resolveTreeKey };
};
