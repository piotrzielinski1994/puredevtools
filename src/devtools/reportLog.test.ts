...
describe('reduceLog', () => {
  it('should append an entry carrying the report fields with a monotonic id starting at 1 (TC-004)', () => {
    const report = buildReport({ kind: 'rewrite', method: 'POST', url: 'https://x/y', status: 418, body: 'hi' });
    ...
    const afterSecond = reduceLog(afterFirst, { type: 'report', report: buildReport() });
  ...
  it('should cap entries at MAX_ENTRIES dropping the oldest while keeping the newest (TC-005)', () => {
    ...
    for (let index = 0; index < total; index += 1) {
      state = reduceLog(state, { type: 'report', report: buildReport({ url: `https://x/${index}` }) });
    }
  ...
  it('should empty the entries on a clear action (TC-006)', () => {
    const withEntry = reduceLog(emptyLog(), { type: 'report', report: buildReport() });
    const cleared = reduceLog(withEntry, { type: 'clear' });
    expect(cleared.entries).toEqual([]);
  });

  it('should not mutate the input state when reducing a report (purity)', () => {
    const original = reduceLog(emptyLog(), { type: 'report', report: buildReport() });
