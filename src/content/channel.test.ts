...
describe("isRulesSyncMessage", () => {
  ...
  it("should reject a message with the wrong source", () => {
    expect(
      isRulesSyncMessage({ source: "other", rules: [], globalEnabled: true }),
    ).toBe(false);
  });
...
describe("isReportChannelMessage", () => {
  ...
  it("should accept a well-formed report message", () => {
    expect(isReportChannelMessage({ source: REPORT_CHANNEL, report })).toBe(
      true,
    );
  });
  ...
  it("should reject a wrong source or a missing report object", () => {
    ...
    expect(
      isReportChannelMessage({ source: REPORT_CHANNEL, report: null }),
    ).toBe(false);
