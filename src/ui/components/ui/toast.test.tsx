...
const Trigger = ({ message, variant }: { message: string; variant?: 'success' | 'error' }) => {
  const { show } = useToast();
  return (
...
describe('ToastProvider', () => {
  ...
  it('should apply the text-destructive class if the error variant is used', () => {
    ...
    expect(screen.getByText('Import failed: boom').closest('.text-destructive')).not.toBeNull();
  });
  ...
  it('should apply the text-emerald-600 class if the success variant is used', () => {
    ...
    expect(screen.getByText('Rules imported.').closest('.text-emerald-600')).not.toBeNull();
  });
