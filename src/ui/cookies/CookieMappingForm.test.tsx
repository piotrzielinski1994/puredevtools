...
describe("CookieMappingForm", () => {
  ...
  it("should patch the source url on edit", () => {
    ...
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({ sourceUrl: "https://x.com" }),
    );
  });
  ...
  it("should patch the target url on edit", () => {
    ...
    fireEvent.change(screen.getByLabelText(/target url/i), {
      target: { value: "http://localhost:9" },
    });
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({ targetUrl: "http://localhost:9" }),
    );
  });

  it("should parse comma separated cookie names into a trimmed array", () => {
    const { onChange } = renderForm({ cookieNames: [] });
    fireEvent.change(screen.getByLabelText(/cookie names/i), {
      target: { value: "auth,  sid , refresh" },
    });
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({ cookieNames: ["auth", "sid", "refresh"] }),
    );
  });
