  ...
  <nav aria-label="Cookie mappings" className="min-h-0 flex-1 overflow-y-auto">
    <ul className="flex list-none flex-col p-0">
      {mappings.map((mapping) => (
        <li key={mapping.id} className="border-b border-b-border last:border-b-0">
          <button
            type="button"
          ...
          >
            <span className="truncate text-sm font-medium">{mapping.name || '(unnamed mapping)'}</span>
            <span className="truncate font-mono text-xs text-muted-foreground">{subtitle(mapping)}</span>
          </button>
        </li>
      ))}
    </ul>
