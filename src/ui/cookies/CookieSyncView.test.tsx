...
const sidebar = () =>
  screen.getByRole("navigation", { name: /cookie mappings/i });
...
describe("CookieSyncView", () => {
  ...
  it("should list every mapping as a selectable sidebar row (TC-011)", async () => {
    renderView(
      createFakeGateway([
        mapping(),
        mapping({ id: "cm2", name: "staging -> local" }),
      ]),
    );
    const nav = await screen.findByRole("navigation", {
      name: /cookie mappings/i,
    });
    expect(
      within(nav).getByRole("button", { name: /prod -> local/i }),
    ).toBeInTheDocument();
    expect(
      within(nav).getByRole("button", { name: /staging -> local/i }),
    ).toBeInTheDocument();
  });
  ...
  it("should show the source and target url as the sidebar row subtitle (TC-011)", async () => {
    ...
    expect(
      within(nav).getByText(/app\.prod\.com.*localhost:3000/i),
    ).toBeInTheDocument();
  });
  ...
  it("should show the selected mapping name and its actions in the content top bar (TC-011)", async () => {
    ...
    const region = detail();
    expect(
      await within(region).findByText("prod -> local"),
    ).toBeInTheDocument();
    expect(
      within(region).getByRole("button", { name: /sync now/i }),
    ).toBeInTheDocument();
    expect(
      within(region).getByRole("button", { name: /delete mapping/i }),
    ).toBeInTheDocument();
  });
  ...
  it("should add a mapping from the content tab bar, not the sidebar (TC-011)", async () => {
    ...
    const region = detail();
    fireEvent.click(
      await within(region).findByRole("button", { name: /add mapping/i }),
    );
    await waitFor(() =>
      expect(savedMappings(gateway.saved.at(-1)).length).toBe(1),
    );
    expect(
      within(sidebar()).queryByRole("button", { name: /add mapping/i }),
  ...
  it("should switch the detail form when another sidebar row is selected (TC-011)", async () => {
    ...
    await waitFor(() =>
      expect(within(detail()).getByLabelText(/mapping name/i)).toHaveValue(
        "staging -> local",
      ),
    );
  });
  ...
  it("should add a new mapping, persist it, and select it (TC-011)", async () => {
    ...
    expect(
      within(detail()).getByLabelText(/mapping name/i),
    ).toBeInTheDocument();
  });
  ...
  it("should add a new mapping from the empty-area sidebar context menu", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("menuitem", { name: /new mapping/i }),
    );
  ...
  it("should delete the selected mapping and persist the removal (TC-011)", async () => {
    ...
    renderView(gateway);
    fireEvent.click(
      await within(detail()).findByRole("button", { name: /delete mapping/i }),
    );
    await waitFor(() =>
      expect(savedMappings(gateway.saved.at(-1))).toEqual([]),
    );
  });
  ...
  it("should persist an edit to the target url (TC-014)", async () => {
    ...
    await waitFor(() =>
      expect(savedMappings(gateway.saved.at(-1))[0].targetUrl).toBe(
        "http://localhost:4000",
      ),
    );
  });
  ...
  it("should reflect a name edit in the sidebar row label (TC-011)", async () => {
    ...
    await waitFor(() =>
      expect(
        within(sidebar()).getByRole("button", { name: /renamed/i }),
      ).toBeInTheDocument(),
    );
  });
  ...
  it("should parse comma separated cookie names into a trimmed array (TC-011)", async () => {
    ...
    await waitFor(() =>
      expect(savedMappings(gateway.saved.at(-1))[0].cookieNames).toEqual([
        "auth",
        "sid",
        "refresh",
      ]),
  ...
  it("should call sync and show a copied-count toast on Sync now (TC-013)", async () => {
    ...
    renderView(gateway);
    fireEvent.click(
      await within(detail()).findByRole("button", { name: /sync now/i }),
    );
  ...
  it("should report each skipped cookie name and reason in the toast (TC-013)", async () => {
    ...
    expect(
      await screen.findByText(
        /skipped 2: sid \(not found on source\), __Host-csrf \(rejected by browser\)/i,
      ),
    ).toBeInTheDocument();
  });
...
describe("CookieSyncView folders", () => {
  it("should render a nested folder/mapping tree to arbitrary depth (TC-005)", async () => {
    renderView(
      createTreeGateway([
        cookieFolder("env", [
          cookieFolder("prod", [mappingNode({ id: "cm1", name: "auth" })]),
        ]),
      ]),
    );
  ...
  it("should offer Edit / Duplicate / Delete on a mapping context menu (TC-007)", async () => {
    const gateway = createTreeGateway([
    ...
    await waitFor(() =>
      expect(savedMappings(gateway.saved.at(-1))).toHaveLength(2),
    );
  });
  ...
  it("should add a folder from the empty-area context menu (TC-008)", async () => {
    ...
    await waitFor(() =>
      expect(
        gateway.saved.at(-1)?.tree.some((node) => node.kind === "folder"),
      ).toBe(true),
    );
  });
  ...
  it("should delete a folder and its whole subtree after a confirm (TC-011)", async () => {
    ...
    const gateway = createTreeGateway([
      cookieFolder("env", [mappingNode({ id: "cm1" })]),
    ]);
    renderView(gateway);
    fireEvent.contextMenu(await screen.findByLabelText("Folder: env"));
  ...
  it("should hide a collapsed folder children and persist the collapsed flag when toggled (TC-010)", async () => {
    const gateway = createTreeGateway([
      cookieFolder("env", [mappingNode({ id: "cm1", name: "auth" })], true),
    ]);
    renderView(gateway);
    ...
    expect(
      savedFolder && savedFolder.kind === "folder"
        ? savedFolder.collapsed
        : null,
    ).toBe(false);
  });
});
