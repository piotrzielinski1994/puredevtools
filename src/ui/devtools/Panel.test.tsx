...
const portMock = vi.hoisted(() => {
  ...
  return {
    ...
    connect: vi.fn(() => ({
      postMessage: vi.fn(),
      onMessage: {
        addListener: (listener: (message: unknown) => void) => portMock.listeners.add(listener),
        removeListener: (listener: (message: unknown) => void) => portMock.listeners.delete(listener),
      },
      disconnect: vi.fn(),
    })),
    push(report: InterceptReport) {
      portMock.listeners.forEach((listener) => listener({ type: 'report', report }));
    },
  };
});
