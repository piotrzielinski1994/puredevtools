...
const DetailSection = ({
  ...
  <section>
    <h3 className="border-b px-3 py-1.5 text-xs font-semibold text-muted-foreground">
      {title}
    </h3>
    <pre className="overflow-auto whitespace-pre-wrap wrap-break-word p-3 font-mono text-xs">
      {children}
    </pre>
  </section>
);

export const InterceptTable = ({
  entries,
  onClear,
  filterInputRef,
}: InterceptTableProps) => {
        ...
        <div className="min-w-0 flex-1 overflow-auto">
            ...
            <tbody>
              {visible.map((entry) => (
                ...
                >
                  ...
                  <td
                    className={`px-3 py-1.5 font-mono text-xs ${statusTone(entry.status)}`}
                  >
                    {entry.status}
                  </td>
                  <td
          ...
          {entries.length === 0 ? (
            <div className="flex items-center justify-center p-8 text-center text-sm text-muted-foreground">
              No intercepted requests yet. Requests matched by a
              response-override rule appear here.
            </div>
          ) : null}
            ...
            <div className="flex items-start justify-between gap-2 border-b px-3 py-2">
              ...
              <div className="flex shrink-0 gap-1">
                ...
                <Button
                  ...
                  onClick={() =>
                    void navigator.clipboard?.writeText(toCurl(selected))
                  }
                >
                  cURL
                </Button>
            ...
            <div className="min-h-0 flex-1 overflow-auto">
              ...
              {selected.requestBody ? (
                <DetailSection title="Request body">
                  {prettyBody(selected.requestBody)}
                </DetailSection>
              ) : null}
