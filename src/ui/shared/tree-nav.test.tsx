...
describe("SidebarTree roving-focus navigation", () => {
  ...
  it("should move the tabbable row to follow the navigation", async () => {
    ...
    await waitFor(() =>
      expect(row("Folder: Beta")).toHaveAttribute("tabindex", "0"),
    );
  ...
  it("should expand a collapsed folder if ArrowRight", async () => {
    ...
    await waitFor(() =>
      expect(gateway.toggleCollapse).toHaveBeenCalledWith("Alpha"),
    );
  });
  ...
  it("should reorder a row down among siblings if Alt+ArrowDown", async () => {
    ...
    const [dragId, target] = gateway.moveNode.mock.calls.at(-1) as [
      string,
  ...
  it("should duplicate the focused rule if the duplicate shortcut is pressed", async () => {
    ...
    expect(gateway.duplicateRule.mock.calls.at(-1)?.[0]).toMatchObject({
      id: "top",
    });
  });
  ...
  it("should begin renaming the focused folder if the rename shortcut is pressed", async () => {
    ...
    expect(
      await screen.findByRole("textbox", { name: /rename folder/i }),
    ).toBeInTheDocument();
  });
});
