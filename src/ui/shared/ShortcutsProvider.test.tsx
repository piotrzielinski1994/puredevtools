...
import { ShortcutsProvider, useShortcutOverrides, useShortcuts } from './ShortcutsProvider';
...
const SAVE_DEFAULT = SHORTCUT_ACTIONS.find((a) => a.id === 'save-rule')!.defaultHotkey;
...
const Probe = () => {
  ...
  return (
    <div>
      <span data-testid="save-rule">{binding === undefined ? 'none' : JSON.stringify(binding)}</span>
      ...
      <button type="button" onClick={() => removeShortcut('save-rule', 'Mod+Y')}>
        remove added
      </button>
      <button type="button" onClick={() => removeShortcut('save-rule', SAVE_DEFAULT)}>
        remove default
      </button>
      <button type="button" onClick={() => replaceShortcut('save-rule', SAVE_DEFAULT, 'Mod+Y')}>
        replace default
      </button>
      <button type="button" onClick={() => replaceShortcut('save-rule', 'Mod+X', 'Mod+Y')}>
        replace absent
      </button>
...
describe('ShortcutsProvider mutators', () => {
  ...
  it('should seed the default then append the new binding if addShortcut is called', async () => {
    ...
    await waitFor(() => expect(value()).toBe(JSON.stringify([SAVE_DEFAULT, 'Mod+Y'])));
  });
  ...
  it('should persist the appended override via storage.local.set', async () => {
    ...
    await waitFor(() => {
      const persisted = mock.set.mock.calls.at(-1)?.[0] as Record<string, unknown> | undefined;
      expect(persisted?.[STORAGE_KEYS.shortcuts]).toEqual({ 'save-rule': [SAVE_DEFAULT, 'Mod+Y'] });
    });
  });
  ...
  it('should swap one binding in place if replaceShortcut is called', async () => {
    ...
    await waitFor(() => expect(value()).toBe(JSON.stringify([SAVE_DEFAULT, 'Mod+G'])));

    await user.click(screen.getByRole('button', { name: /replace default/i }));

    await waitFor(() => expect(value()).toBe(JSON.stringify(['Mod+Y', 'Mod+G'])));
  });
  ...
  it('should persist the removal of the override key if resetShortcut is called', async () => {
    ...
    await waitFor(() => {
      const persisted = mock.set.mock.calls.at(-1)?.[0] as Record<string, unknown> | undefined;
      const stored = persisted?.[STORAGE_KEYS.shortcuts] as Record<string, unknown> | undefined;
...
describe('ShortcutsProvider live sync', () => {
  ...
  it('should reflect a change delivered through storage.onChanged', async () => {
    ...
    mock.emit({ [STORAGE_KEYS.shortcuts]: { newValue: { 'save-rule': ['Mod+Y'] } } });
  ...
  it('should ignore a change on a non-local storage area', async () => {
    ...
    mock.emit({ [STORAGE_KEYS.shortcuts]: { newValue: { 'save-rule': ['Mod+Y'] } } }, 'sync');
