...
describe("SidebarTree rendering", () => {
  it("should render a nested tree of folders and rules to arbitrary depth (AC-001, TC-018)", async () => {
    renderTree([
      folder("outer", [folder("inner", [ruleNode("deep")]), ruleNode("mid")]),
      ruleNode("top"),
    ]);

    expect(
      await screen.findByRole("button", { name: "Edit: deep" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Edit: mid" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Edit: top" }),
    ).toBeInTheDocument();
  ...
  it("should summarize request-side and script actions without leaving empty commas", async () => {
        ...
        actions: [
          {
            ...
            headers: [{ op: "remove", name: "Authorization" }],
          },
    ...
    expect(summary).toHaveTextContent(
      "request headers, request url, pre-script",
    );
  ...
  it("should label response-side actions in the rule subtitle", async () => {
    ...
    expect(summary).toHaveTextContent(
      "response headers, response body, post-script",
    );
  });
});
...
describe("SidebarTree folder CRUD", () => {
  ...
  it("should create a folder inside a folder from its context menu (AC-007)", async () => {
    ...
    await waitFor(() =>
      expect(gateway.addFolder).toHaveBeenCalledWith("parent"),
    );
  });
  ...
  it("should rename a folder via the context menu inline input (AC-008)", async () => {
    ...
    await waitFor(() =>
      expect(gateway.renameFolder).toHaveBeenCalledWith("f", "Auth"),
    );
  });
  ...
  it("should cancel a rename on Escape without calling renameFolder (AC-008)", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.queryByRole("textbox", { name: /rename folder/i }),
      ).not.toBeInTheDocument(),
    );
...
describe("SidebarTree collapse", () => {
  it("should call toggleCollapse and hide children when a folder row is clicked (AC-010)", async () => {
    ...
    const row = await screen.findByLabelText("Folder: f");
    expect(
      screen.getByRole("button", { name: "Edit: child" }),
    ).toBeInTheDocument();

    fireEvent.click(row);

    await waitFor(() =>
      expect(gateway.toggleCollapse).toHaveBeenCalledWith("f"),
    );
    await waitFor(() =>
      expect(
        screen.queryByRole("button", { name: "Edit: child" }),
      ).not.toBeInTheDocument(),
    );
  });
...
const dragTo = (
  ...
  fireEvent.pointerDown(source, {
    ...
    isPrimary: true,
    clientX: 10,
    clientY: sourceTop + 10,
  });
  fireEvent.pointerMove(document, {
    pointerId: 1,
    clientX: 10,
    clientY: sourceTop + 20,
  });
...
describe("SidebarTree drag-and-drop", () => {
  it("should call moveNode when a rule is dropped before another (AC-002)", async () => {
    ...
    const [dragId, target] = gateway.moveNode.mock.calls[0] as [
      string,
  ...
  it("should render an empty-folder drop zone while a drag is active (TC-003)", async () => {
    ...
    fireEvent.pointerDown(rowFor("r1"), {
      pointerId: 1,
      button: 0,
      isPrimary: true,
      clientX: 10,
...
describe("SidebarTree search filter", () => {
  it("should render a flat filtered list with no folders or drag handles while filtering", async () => {
    renderTree(
      [folder("mine", [ruleNode("alpha"), ruleNode("beta")])],
      vi.fn(),
      "alpha",
    );
    ...
    const row = screen
      .getByRole("button", { name: "Edit: alpha" })
