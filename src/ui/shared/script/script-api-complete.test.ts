...
const REQ_READONLY = ['getUrl', 'getMethod', 'getHeader', 'getHeaders', 'getBody'];
...
describe('apiMembers (AC-012)', () => {
  it('should list the full req read+write set in the pre stage', () => {
    ...
    REQ_FULL.forEach((name) => expect(members).toContain(name));
  });
  ...
  it('should list only the read-only req getters in the post stage', () => {
    ...
    REQ_READONLY.forEach((name) => expect(members).toContain(name));
  ...
  it('should list the response members in the post stage', () => {
    ...
    RES_MEMBERS.forEach((name) => expect(members).toContain(name));
  });
  ...
  it('should list the console methods in either stage', () => {
    ...
    expect(apiMembers('console', 'pre')).toEqual(['log', 'info', 'warn', 'error']);
    expect(apiMembers('console', 'post')).toEqual(['log', 'info', 'warn', 'error']);
  });
...
describe('scriptApiCompletion (AC-012)', () => {
  ...
  it('should offer the top-level namespaces for a bare word, stage-aware', () => {
    ...
    expect(pre!.options.map((option) => option.label)).toEqual(['req', 'console']);
    expect(post!.options.map((option) => option.label)).toEqual(['req', 'res', 'console']);
  });
