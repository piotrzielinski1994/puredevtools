...
const makeId = (seed: string): string =>
  `rule-${seed.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-${seed.length}`;

const opToRow = (op: HeaderOp): OpRow =>
  op.op === "set"
    ? { op: "set", name: op.name, value: op.value }
    : { op: "remove", name: op.name, value: "" };

const rowToOp = (row: OpRow): HeaderOp =>
  row.op === "set"
    ? { op: "set", name: row.name, value: row.value }
    : { op: "remove", name: row.name };
...
export const ruleToDraft = (rule: Rule): RuleDraft => ({
  ...
  responseOps: (firstAction(rule, "modifyResponseHeaders")?.headers ?? []).map(
    opToRow,
  ),
  rewriteBody: firstAction(rule, "rewriteBody")?.body ?? "",
  requestOps: (firstAction(rule, "modifyRequestHeaders")?.headers ?? []).map(
    opToRow,
  ),
...
const buildActions = (draft: RuleDraft): RuleAction[] => {
  const respOps = draft.responseOps
    .filter((row) => row.name.trim() !== "")
    .map(rowToOp);
  const reqOps = draft.requestOps
    .filter((row) => row.name.trim() !== "")
    .map(rowToOp);
  const actions: RuleAction[] = [];
  if (respOps.length > 0)
    actions.push({ type: "modifyResponseHeaders", headers: respOps });
  if (draft.rewriteBody.trim() !== "")
    actions.push({ type: "rewriteBody", body: draft.rewriteBody });
  if (reqOps.length > 0)
    actions.push({ type: "modifyRequestHeaders", headers: reqOps });
  if (draft.requestBody.trim() !== "")
    actions.push({ type: "rewriteRequestBody", body: draft.requestBody });
  if (draft.requestUrl.trim() !== "")
    actions.push({ type: "rewriteRequestUrl", target: draft.requestUrl });
  if (draft.preScript.trim() !== "")
    actions.push({ type: "preScript", source: draft.preScript });
  if (draft.postScript.trim() !== "")
    actions.push({ type: "postScript", source: draft.postScript });
  return actions;
};

export const draftToRule = (
  draft: RuleDraft,
  baseline?: Rule,
): DraftToRuleResult => {
  if (draft.pattern.trim() === "")
    return { ok: false, error: "URL pattern is required." };
  if (draft.kind === "regex" && !isValidRegex(draft.pattern))
