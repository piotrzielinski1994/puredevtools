...
const createGatewayWith = (rules: Rule[]): FakeGateway =>
  createFakeGateway(ruleNodes(rules));
...
describe("ImportExportControls", () => {
  ...
  it("should call importFromFile with the file text and replace and show a success toast if confirmed (AC-003, TC-002)", async () => {
    ...
    fireEvent.change(importFileInput(), {
      target: { files: [jsonFile(VALID_JSON)] },
    });

    await waitFor(() =>
      expect(gateway.importFromFile).toHaveBeenCalledTimes(1),
    );
  ...
  it("should show an error toast if the file cannot be read (edge)", async () => {
    ...
    Object.defineProperty(file, "text", {
      value: () => Promise.reject(new Error("read boom")),
    });
