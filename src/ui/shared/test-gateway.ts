...
export const createFakeGateway = (
  ...
  const gateway = {
    ...
    addRule: vi.fn<(rule: Rule) => Promise<void>>(async (rule) => {
      store = insertNode(store, ruleNode(rule), {
        parentId: null,
        index: store.length,
      });
    }),
    ...
    duplicateRule: vi.fn<(rule: Rule, newId: string) => Promise<void>>(
      async (rule, newId) => {
        store = insertNode(store, ruleNode(cloneRule(rule, newId)), {
          parentId: null,
          index: store.length,
        });
      },
    ...
    moveNode: vi.fn<(dragId: string, target: MoveTarget) => Promise<void>>(
      async (dragId, target) => {
        store = moveTreeNode(store, dragId, target);
      },
    ),
    addFolder: vi.fn<(parentId: string | null) => Promise<string>>(
      async (parentId) => {
    ...
    renameFolder: vi.fn<(id: string, name: string) => Promise<void>>(
      async (id, name) => {
    ...
    setGlobalEnabled: vi.fn<(enabled: boolean) => Promise<void>>(
      async (enabled) => {
        global = enabled;
      },
    ),
    ...
    importFromFile: vi
      .fn<(json: string) => Promise<ImportOutcome>>()
