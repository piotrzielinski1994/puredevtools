...
const actionSummary = (node: RuleNode): string => {
  const labels = node.rule.actions.map((action) => ACTION_LABELS[action.type]).filter(Boolean);
...
export const RuleRow = ({
  ...
  return (
    <LeafRow node={node} id={rule.id} ariaLabel={`Rule: ${rule.name}`} depth={depth}>
      <Switch
      ...
      >
        ...
        <p className="truncate text-xs text-muted-foreground">
          ...
          <span data-testid={`rule-action-summary-${rule.id}`}>{actionSummary(node)}</span>
        </p>
      </button>
    </LeafRow>
  );
