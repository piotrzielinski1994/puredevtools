...
const matchesFilter = (rule: Rule, query: string): boolean => {
  ...
  return (
    ...
    rule.matchers.url.pattern.toLowerCase().includes(needle)
  );
};
...
export const SidebarTree = ({
  ...
  const leafMenuItems = (node: RuleNode): ContextMenuItem[] => [
    { label: "Edit", icon: Pencil, onSelect: () => onEdit(node.rule.id) },
    {
      label: "Duplicate",
      icon: Copy,
      onSelect: () => void duplicateRule(node.rule),
    },
    {
      label: "Delete",
      icon: Trash2,
      destructive: true,
      onSelect: () => void removeNode(node.rule.id),
    },
  ];

  const ruleById = (id: string): Rule | undefined =>
  ...
  const adapter: TreeAdapter<RuleNode> = {
    ...
    renderLeaf: (node, depth) => (
      <RuleRow
        node={node}
        depth={depth}
        onToggle={(rule) => void updateRule(rule)}
      />
    ),
