...
vi.mock("webextension-polyfill", () => ({
  default: {
    storage: {
      ...
      onChanged: {
        addListener: storageMock.addListener,
        removeListener: storageMock.removeListener,
      },
    },
  },
}));
...
const getKindSelect = () =>
  screen.getByRole("combobox", { name: /pattern kind/i });
...
const getRewriteRequestUrl = () =>
  screen.getByLabelText(/rewrite request url/i);
...
describe("RuleForm", () => {
  ...
  it("should prefill the response header ops and body from the draft", () => {
    ...
    expect(
      screen.getByLabelText(/Modify response headers name 0/i),
    ).toHaveValue("X-Env");
  });
  ...
  it("should call onDraftChange with the new name if the name field is edited", () => {
    ...
    const { onDraftChange } = setup({
      draft: makeDraft({ pattern: "https://x.test/*" }),
    });
  ...
  it("should call onDraftChange with the new url pattern if the pattern field is edited", () => {
    ...
    fireEvent.change(getUrlPatternInput(), {
      target: { value: "https://api.test.dev/*" },
    });

    expect(onDraftChange).toHaveBeenCalledWith(
      expect.objectContaining({ pattern: "https://api.test.dev/*" }),
    );
  });
  ...
  it("should call onDraftChange with the new body if the body field is edited", () => {
    ...
    expect(onDraftChange).toHaveBeenCalledWith(
      expect.objectContaining({ rewriteBody: "<p>x</p>" }),
    );
  });
  ...
  it("should call onSave when the Mod+S save chord is pressed", async () => {
    ...
    const { onSave } = setup({
      draft: makeDraft({ pattern: "https://api.test.dev/*" }),
    });
  ...
  it("should save on the rebound key if save-rule is overridden", async () => {
    ...
    Object.keys(storageMock.backing).forEach((key) => {
      delete storageMock.backing[key];
    });
  ...
  it("should render the error inline when onSave resolves not ok", async () => {
    ...
    const onSave = vi
      .fn<() => Promise<{ ok: boolean; error?: string }>>()
      .mockResolvedValue({ ok: false, error: "URL pattern is required." });
    setup({ onSave });

    await pressSaveChord();

    expect(
      await screen.findByText(/url pattern is required/i),
    ).toBeInTheDocument();
  });
  ...
  it("should report a positive match in the URL tester", async () => {
    ...
    fireEvent.change(screen.getByLabelText(/test url/i), {
      target: { value: "https://api.test.dev/users" },
    });
  ...
  it("should report a non-match in the URL tester", async () => {
    ...
    fireEvent.change(screen.getByLabelText(/test url/i), {
      target: { value: "https://other.dev/x" },
    });
  ...
  it("should render a Request tab that reveals the request header editor and request body textarea when selected (TC-012)", () => {
    ...
    expect(
      screen.getByRole("button", { name: /add .*request headers/i }),
    ).toBeInTheDocument();
  });
  ...
  it("should prefill the request header ops and request body from the draft (TC-012)", () => {
    ...
    expect(screen.getByLabelText(/request headers name 0/i)).toHaveValue(
      "X-Env",
    );
  });
  ...
  it("should call onDraftChange with the new requestBody if the request body field is edited (TC-012)", () => {
    ...
    expect(onDraftChange).toHaveBeenCalledWith(
      expect.objectContaining({ requestBody: '{"q":9}' }),
    );
  });
  ...
  it("should call onDraftChange with updated requestOps if a request header name is edited (TC-012)", () => {
    ...
    fireEvent.change(screen.getByLabelText(/request headers name 0/i), {
      target: { value: "X-New" },
    });

    expect(onDraftChange).toHaveBeenCalledWith(
      expect.objectContaining({
        requestOps: [{ op: "set", name: "X-New", value: "staging" }],
      }),
    );
  });
  ...
  it("should call onDraftChange with the new requestUrl if the rewrite request URL input is edited (TC-014)", () => {
    ...
    fireEvent.change(getRewriteRequestUrl(), {
      target: { value: "http://localhost:4000/mock" },
    });
  ...
  it("should call onDraftChange with the new preScript when the pre-request editor is edited (AC-012)", async () => {
    ...
    expect(onDraftChange).toHaveBeenCalledWith(
      expect.objectContaining({ preScript: expect.stringContaining("x") }),
    );
  });
});
