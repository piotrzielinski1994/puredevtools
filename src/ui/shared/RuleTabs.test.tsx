...
describe("RuleTabs", () => {
  ...
  it("should expose a close control for every tab (AC-007)", () => {
    ...
    expect(
      screen.getByRole("button", { name: /close alpha rule/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /close bravo rule/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /close new rule/i }),
    ).toBeInTheDocument();
  });
  ...
  it("should render nothing selectable as current if activeKey is null", () => {
    ...
    expect(
      document.querySelector('[aria-current="true"], [aria-current="page"]'),
    ).toBeNull();
  });
  ...
  it("should render the Unsaved changes mark only inside the dirty tab (AC-002)", () => {
    ...
    const dirtyTab = screen
      .getByRole("button", { name: /close alpha rule/i })
      .closest('[role="tab"]');
    const cleanTab = screen
      .getByRole("button", { name: /close bravo rule/i })
      .closest('[role="tab"]');
