...
describe("cookieMappingSchema", () => {
  ...
  it("should parse a mapping with an empty cookieNames array (TC-002)", () => {
    expect(
      cookieMappingSchema.safeParse({ ...validMapping, cookieNames: [] })
        .success,
    ).toBe(true);
  });
});

const mappingNode = (over: Record<string, unknown> = {}) => ({
  kind: "mapping",
  mapping: { ...validMapping, ...over },
});
...
describe("cookieSyncStateSchema", () => {
  ...
  it("should reject a state carrying an unknown key (strict)", () => {
    expect(cookieSyncStateSchema.safeParse({ tree: [], foo: 1 }).success).toBe(
      false,
    );
  });
  ...
  it("should reject a mapping node carrying an unknown field (TC-004, strict)", () => {
    const state = {
      tree: [{ kind: "mapping", mapping: { ...validMapping, extra: 1 } }],
    };
