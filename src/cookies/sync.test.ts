...
const makePort = (
  ...
  const set = vi.fn(
    setImpl ??
      (async (d: Cookies.SetDetailsType) =>
        ({ ...cookie(), ...d }) as Cookies.Cookie),
  );
...
describe("syncMapping", () => {
  ...
  it("should read source cookies using the source URL (TC-004)", async () => {
    ...
    await syncMapping(
      mapping({ sourceUrl: "https://app.prod.com/path" }),
      port,
    );
  ...
  it("should report a requested name absent from source as skipped not-found (TC-005)", async () => {
    ...
    const result = await syncMapping(
      mapping({ cookieNames: ["auth", "missing"] }),
      port,
    );
  ...
  it("should leave a non-None sameSite unchanged when secure is dropped", async () => {
    ...
    await syncMapping(mapping({ targetUrl: "http://localhost:3000" }), port);
    expect((set.mock.calls[0][0] as Cookies.SetDetailsType).sameSite).toBe(
      "strict",
    );
  });
  ...
  it("should omit expirationDate for a session cookie", async () => {
    ...
    expect(
      "expirationDate" in (set.mock.calls[0][0] as Cookies.SetDetailsType),
    ).toBe(false);
  });
  ...
  it("should set each source cookie sharing a name but differing by path (edge)", async () => {
    ...
    expect(set.mock.calls.map((c) => c[0].path).sort()).toEqual([
      "/",
      "/admin",
    ]);
  });
