...
const folder = (id: string, children: CookieTreeNode[] = [], over: Partial<CookieFolderNode> = {}): CookieFolderNode => ({
  kind: 'folder',
  id,
  name: id,
  collapsed: false,
...
describe('flatten', () => {
  it('should return mappings in DFS pre-order across nested folders', () => {
    const tree = [folder('f', [mappingNode({ id: 'cm1' }), folder('g', [mappingNode({ id: 'cm2' })])]), mappingNode({ id: 'cm3' })];
...
describe('migrateLegacy', () => {
  it('should wrap each flat mapping as a root mapping node', () => {
    expect(migrateLegacy([mapping({ id: 'cm1' }), mapping({ id: 'cm2' })])).toEqual([
...
describe('updateMappingInTree', () => {
  it('should replace the matching mapping deep in the tree', () => {
    ...
    const result = updateMappingInTree(tree, mapping({ id: 'cm1', name: 'new' }));
...
describe('moveNode', () => {
  it('should move a mapping into a folder', () => {
    ...
    expect(childrenOf(result[0]).map((n) => (n.kind === 'mapping' ? n.mapping.id : n.id))).toEqual(['cm1']);
  });
});

describe('duplicateNode', () => {
  it('should duplicate a mapping with a fresh id and (copy) name as a sibling', () => {
    const result = duplicateNode([mappingNode({ id: 'cm1', name: 'auth' })], 'cm1');
    expect(result).toHaveLength(2);
    const clone = result[1];
    expect(clone.kind === 'mapping' ? clone.mapping.name : null).toBe('auth (copy)');
  ...
  it('should deep-clone a folder subtree with fresh ids and (copy) on the top only', () => {
    const tree = [folder('folder-1', [mappingNode({ id: 'cm1', name: 'keep' })], { name: 'env' })];
    ...
    expect(child && child.kind === 'mapping' ? child.mapping.name : null).toBe('keep');
    expect(child && child.kind === 'mapping' ? child.mapping.id : null).not.toBe('cm1');
  ...
  it('should not mutate the original when the clone is changed', () => {
    ...
    expect(original.kind === 'mapping' ? original.mapping.cookieNames : null).toEqual(['auth']);
  });
});
